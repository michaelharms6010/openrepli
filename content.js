...
function showLoadingSpinner() {
  ...
  loadingSVG.id = "oneclickreply-spinner";
  ...
  const replyButton = document.querySelector("#oneclickreply-button");
  replyButton.style.visibility = "hidden";
}

function hideLoadingSpinner() {
  const element = document.querySelector("#oneclickreply-spinner");
  element.remove();
...
function showModal(error) {
  ...
  modal.setAttribute("id", "oneclickreply-modal");
  modal.style.cssText = `
      display: block;
      position: fixed;
      padding: 30px;
  ...
  header.innerHTML = "oneclickreply";
  ...
  button.addEventListener("click", function () {
    document.getElementById("oneclickreply-modal").remove();
  });
...
async function injectTwitterButton() {
  ...
  const button = document.createElement("button");
  button.id = "oneclickreply-button";
  button.innerHTML = `
...
function findClosestReplyButton(el) {
  const target = el.querySelector("#oneclickreply-button");
  if (target) {
    return target;
  }
  if (!el.parentElement) {
...
async function injectLinkedInButton() {
  ...
  for (i = 0; i < targetElements.length; i++) {
    ...
    if (parentBox.children.length < 3) {
      ...
      replyButton.id = "oneclickreply-button";
